...
resource "null_resource" "run_custom_script" {
  provisioner "local-exec" {
    command = "bash -c 'mkdir ~/.kube && mkdir ~/.talos && terraform output -raw kubeconfig > ~/.kube/config && terraform output -raw talosconfig > ~/.talos/config && chmod 600 ~/.kube/config ~/.talos/config'"
  }
  triggers = {
